...
public class RainFallView extends View{
	...
	public void setData(List<TrendDto> dataList, int time) {
		...
		if (!dataList.isEmpty()) {
			...
			maxValue = tempList.get(0).rainFall;
			minValue = tempList.get(0).rainFall;
			for (int i = 0; i < tempList.size(); i++) {
				if (maxValue <= tempList.get(i).rainFall) {
	...
	protected void onDraw(Canvas canvas) {
		...
		float chartW = w-CommonUtil.dip2px(mContext, 35);
		float chartH = h-CommonUtil.dip2px(mContext, 40);
		float leftMargin = CommonUtil.dip2px(mContext, 25);
