 ...
 * 雷暴云图下载
 */
public class LeibaoManager {

	private Context mContext;
	private LoadThread mLoadThread;

	public interface LeibaoListener {
	...
	public LeibaoManager(Context context) {
		mContext = context.getApplicationContext();
	}
	
	public void loadImagesAsyn(Map<String, StrongStreamDto> radars, LeibaoListener listener) {
		if (mLoadThread != null) {
			mLoadThread.cancel();
			mLoadThread = null;
		}
	...
	private class LoadThread extends Thread {
		...
		private LeibaoListener listener;
		private int count;
		
		private LoadThread(Map<String, StrongStreamDto> radars, LeibaoListener listener) {
			this.radars = radars;
		...
		private String decodeFromUrl(String url, String startTime){
		    try {
				...
				try {
					File file = new File(getDir() + "/leibao"+startTime+".png");
		...
		private synchronized void finished(String path, Map<String, StrongStreamDto> radars) {
			...
			if (listener != null) {
				...
				if (count <= 0) {
					listener.onResult(radars == null ? LeibaoListener.RESULT_FAILED : LeibaoListener.RESULT_SUCCESSED, radars);
				}
			}
		}
