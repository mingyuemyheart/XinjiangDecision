...
public class WeatherStaticsActivity extends BaseActivity implements OnClickListener, OnMarkerClickListener,
	...
	private String locationCityId = "101130101";//默认为乌鲁木齐
				...
				if (!content.isNull("geo")) {
					try {
						...
						if (!geo.isNull("id")) {
							...
							if (!locationCityId.startsWith("10113")) {
								locationCityId = "101130101";
							}
							OkHttpStatistic();
						}
		...
		try {
			...
			if (!obj.isNull(level)) {
				...
				for (int i = 0; i < array.length(); i++) {
					...
					if (!TextUtils.isEmpty(dto.areaId) && dto.areaId.startsWith("10113")) {
						list.add(dto);
					}
				}
			}
