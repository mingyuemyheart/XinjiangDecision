...
class WarningFragment : BaseFragment(), OnClickListener, OnMapClickListener, OnMarkerClickListener, InfoWindowAdapter {
    ...
    private fun addColumn() {
        ...
        for (i in 0 until size) {
            ...
            tvName.setPadding(0, CommonUtil.dip2px(activity, 10f).toInt(), 0, CommonUtil.dip2px(activity, 10f).toInt())
            tvName.maxLines = 1
            ...
            if (i == 0) {
                tvName.setTextColor(ContextCompat.getColor(activity!!, R.color.colorPrimary))
                tvBar.setBackgroundColor(ContextCompat.getColor(activity!!, R.color.colorPrimary))
            } else {
                tvName.setTextColor(ContextCompat.getColor(activity!!, R.color.text_color3))
                tvBar.setBackgroundColor(ContextCompat.getColor(activity!!, R.color.light_gray))
            }
                ...
                if (llContainer != null) {
                    for (j in 0 until llContainer!!.childCount) {
                        ...
                        if (TextUtils.equals(tvName.text.toString(), name.text.toString())) {
                            name.setTextColor(ContextCompat.getColor(activity!!, R.color.colorPrimary))
                            bar.setBackgroundColor(ContextCompat.getColor(activity!!, R.color.colorPrimary))
                            warningName = tvName.text.toString()
                            addWarnings()
                        } else {
                            name.setTextColor(ContextCompat.getColor(activity!!, R.color.text_color3))
                            bar.setBackgroundColor(ContextCompat.getColor(activity!!, R.color.light_gray))
                        }
                    }
                }
            }
                        ...
                        if (!TextUtils.isEmpty(result)) {
                            try {
                                ...
                                try {
                                    ...
                                    val builderSpan4 = ForegroundColorSpan(ContextCompat.getColor(activity!!, R.color.text_color4))
    ...
    private fun initListViewStatistic() {
        statisticAdapter = WarningStatisticAdapter(activity, statisticList)
    ...
    private fun initListView() {
        warningAdapter = WarningAdapter(activity, dataList, false)
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.tvList -> startActivity(Intent(activity, WarningListActivity::class.java))
            R.id.clHistory -> startActivity(Intent(activity, WarningHistoryActivity::class.java))
