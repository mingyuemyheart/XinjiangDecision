...
public class FactManager {
	...
	public interface RadarListener {
		...
		void onResult(int result, ArrayList<StationMonitorDto> images);
	...
	public void loadImagesAsyn(ArrayList<StationMonitorDto> radars, RadarListener listener) {
		if (mLoadThread != null) {
			mLoadThread.cancel();
			mLoadThread = null;
		}
	...
	private class LoadThread extends Thread {
		private ArrayList<StationMonitorDto> radars;
		private RadarListener listener;
		private int count;
		
		public LoadThread(ArrayList<StationMonitorDto> radars, RadarListener listener) {
			this.radars = radars;
		...
		public void run() {
			...
			for (int i = 0; i < len ; i++) {
				StationMonitorDto radar = radars.get(i);
				String url = radar.imgUrl;
				loadImage(i, url, radars);
			}
		}
		
		private void loadImage(final int index, final String url, final ArrayList<StationMonitorDto> radars) {
		...
		private synchronized void finished(String path, ArrayList<StationMonitorDto> radars) {
			int max = radars.size();
			count -- ;
