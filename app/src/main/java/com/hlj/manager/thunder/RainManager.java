...
public class RainManager {
	...
	public void loadImagesAsyn(Map<String, StrongStreamDto> radars, RainListener listener) {
		if (mLoadThread != null) {
			mLoadThread.cancel();
			mLoadThread = null;
		}
	...
	private class LoadThread extends Thread {
		...
		private RainListener listener;
		private int count;
		
		private LoadThread(Map<String, StrongStreamDto> radars, RainListener listener) {
			this.radars = radars;
		...
		private String decodeFromUrl(String url, String startTime){
		    try {
				...
				try {
					File file = new File(getDir() + "/rain"+startTime+".png");
		...
		private synchronized void finished(String path, Map<String, StrongStreamDto> radars) {
			...
			if (listener != null) {
				...
				if (count <= 0) {
					listener.onResult(radars == null ? RainListener.RESULT_FAILED : RainListener.RESULT_SUCCESSED, radars);
				}
			}
		}
