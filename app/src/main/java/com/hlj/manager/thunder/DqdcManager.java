...
public class DqdcManager {
	...
	public void loadImagesAsyn(Map<String, StrongStreamDto> radars, DqdcListener listener) {
		if (mLoadThread != null) {
			mLoadThread.cancel();
			mLoadThread = null;
		}
	...
	private class LoadThread extends Thread {
		...
		private LoadThread(Map<String, StrongStreamDto> radars, DqdcListener listener) {
			this.radars = radars;
		...
		private String decodeFromUrl(String url, String startTime){
		    try {
				...
				try {
					File file = new File(getDir() + "/dqdc"+startTime+".png");
		...
		private synchronized void finished(String path, Map<String, StrongStreamDto> radars) {
			...
			if (listener != null) {
				...
				if (count <= 0) {
					listener.onResult(radars == null ? DqdcListener.RESULT_FAILED : DqdcListener.RESULT_SUCCESSED, radars);
				}
			}
		}
